...
class WhereAnswer(ans.Answer):
    """Where answer: handles figuring out what block group the person might be in"""
    ...
    def setup(cls,pathToData):
        #Download shapefiles for BlockGroups in all States.
        #From https://www.census.gov/cgi-bin/geo/shapefiles2011/main

        print "Setting up shapefile database in %s" % pathToData
        ...
        cur.execute('CREATE TABLE IF NOT EXISTS boundaries (id INTEGER PRIMARY KEY AUTOINCREMENT, bbox0 DECIMAL(9,6), bbox1 DECIMAL(9,6), bbox2 DECIMAL(9,6), bbox3 DECIMAL(9,6), state CHAR(2), county CHAR(3), tract CHAR(6), blockgroup CHAR(4), geoid CHAR(15), shape VARCHAR(50000));')
        ...
        print "Extracting blockgroup shapefiles."
        for state in cls._states:
            ...
            try:
                ...
                #note: switched to 2010 blockgroups, from url = 'http://www2.census.gov/geo/tiger/TIGER2011/BG/tl_2011_%s_bg.zip' % stateid
